...
import { Input as InputB, type InputProps } from '@/components/ui/input-b';
...
const InputComponent = React.forwardRef<HTMLInputElement, InputProps>(({ className, ...props }, ref) => (
  <InputB className={cn('rounded-e-lg rounded-s-none border-none px-4', className)} {...props} ref={ref} />
));
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          ...
          variant={'outline'}
          className={cn(
            'flex gap-1 rounded-e-none rounded-s-lg px-3 h-full bg-transparent hover:bg-transparent hover:text-white border-0 border-r flex-none w-22',
          )}
          disabled={disabled}
        >
